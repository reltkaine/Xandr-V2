...
/*
  This project is a good example of how to use the AVR chip timeris.
*/
...
/* This is kind of a strange but very useful helper function
  Because we are using compression, we index to the timeri table
...
void ir_tvb_gone_setup() {
  ...
  display.print("IR TV GoNE");

  // determine region
  if (ir_reginal == 1) {
    region = NA;
    display.setCursor(24, 10);
    display.print("REGION NA");
...
int sendAllCodes() {
  ...
  for (i = 0; i < num_codes; i++) {
    ...
    // set OCR for timeri1 to output this POWER code's carrier frequency
    ...
    // Get the number of bits we use to index into the timeri table
...
return 0;
}  //return 1;  end of sendAllCodes

void ir_tvb_gone_loop() {
...
void delay_ten_us(uint16_t us) {
  ...
  while (us != 0) {
    ...
    for (timeri = 0; timeri <= DELAY_CNT; timeri++) {
      NOP;
      NOP;
    }
    NOP;
